...
public class LitigationDocumentsDaoImpl implements LitigationDocumentsDao {
	...
	// Method Created : Akash Jadhav
		// Method Purpose : Get last generated value for completion documents
		@Override
		public int getLastGeneratedValueByCompId(Object litigation_id) {
			try {
				String sql = "SELECT MAX(comp_last_generated_value_for_contract_id) FROM trn_completion_documents where comp_licences_id="
						+ litigation_id;
				Query query = em.createNativeQuery(sql);
		...
		// Method Created : Akash Jadhav
		// Method Purpose : Save Completion documents
		@Override
		public void saveCompletionDocuments(CompletionDocuments completionDocuments) {
			try {
				em.persist(completionDocuments);
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		// Method Created : Akash Jadhav
				// Method Purpose : Get last generated value for completion documents for edit
				@Override
				public int getLastGeneratedValueByCompIdEdit(Object litigation_id) {
					try {
						String sql = "SELECT MAX(comp_last_generated_value_for_contract_id) FROM trn_completion_documents where comp_licences_id="
								+ litigation_id;
						Query query = em.createNativeQuery(sql);
				...
				// Method Created : Akash Jadhav
				// Method Purpose : Save Completion documents for edit
				@Override
				public void saveCompletionDocumentsEdit(CompletionDocuments completionDocuments) {
					try {
						em.persist(completionDocuments);
					} catch (Exception e) {
						e.printStackTrace();
